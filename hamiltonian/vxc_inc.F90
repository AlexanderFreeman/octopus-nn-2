...
subroutine xc_get_vxc(der, xcs, st, rho, ispin, ioniz_pot, qtot, vxc, ex, ec, deltaxc, vtau)
  ...
  FLOAT :: enc4_W(100, 101)
  FLOAT :: encout_W(1, 101)
  FLOAT :: dec1_W(100, 6)
  FLOAT :: dec2_W(100, 101)
  ...
  FLOAT :: laplacians_mean, laplacians_std
  FLOAT :: density_fixed, sigma_fixed, scalars_fixed, laplacians_fixed
  integer :: i,j, k, n1, n2, n3, idx, curr_idx, iter, flatten_idx, iix, iiy, iiz, iip, n_global
  ...
  FLOAT, allocatable :: ec_per_part(:)  ! Correlation energy per unit particle
  !  First order (functional) derivatives
  ...
  n1 = int(der%mesh%np**(1.0/3.0))
  n2 = int(der%mesh%np**(1.0/3.0))
  n3 = int(der%mesh%np**(1.0/3.0))
  
  n_global = size(dens, 1)
  
  SAFE_ALLOCATE(input_sigma(n_global, 1))
  SAFE_ALLOCATE(input_sigma_grad(der%mesh%np, der%mesh%sb%dim, 1))
  SAFE_ALLOCATE(input_scalars(der%mesh%np, 1))
  SAFE_ALLOCATE(input_laplacian(der%mesh%np, 1))
  
  !call random_number(input_sigma)
  ...
  do isp = 1, spin_channels 
	call dderivatives_grad(der, input_sigma(:, isp), input_sigma_grad(:, :, isp)) 
  end do
  
  ! получаем лапласиан
  
  do isp = 1, spin_channels 
	call dderivatives_lapl(der, dens(:, isp), input_laplacian(:, isp)) 
  end do
  
  ! получаем scalars
  
  do ip = 1, der%mesh%np
	  input_scalars(ip, 1) = gdens(ip, 1, 1)*input_sigma_grad(ip, 1, 1) + gdens(ip, 2, 1)*input_sigma_grad(ip, 2, 1) + gdens(ip, 3, 1)*input_sigma_grad(ip, 3, 1)
  end do
    ...
    functl_loop: do ixc = FUNC_X, FUNC_C
      ...
      if(calc_energy) then
        if(functl(ixc)%type == XC_EXCHANGE) then
          ...
          !end do
		  do ib = 1, n_block
			!enc_inp_matrix_gga(1,1) = dens(ib + ip - 1, 1)
			!enc_inp_matrix_gga(2,1) = input_sigma(ib + ip - 1, 1)
			!call gga_encoder(enc_inp_matrix_gga, enc1_W, enc2_W, enc3_W, enc4_W, encout_W, enc_out_matrix_gga)
			!ec_per_vol(ib + ip - 1) = ec_per_vol(ib + ip - 1) + sum(l_dens(1:spin_channels, ib))*enc_out_matrix_gga(1,1)
			ec_per_vol(ib + ip - 1) = 0
		  end do
        end if
      end if
  ...
  ! read gga weights
  if (gga) then
	!open (42, file = 'fc1_W_gga.txt', status = 'old') 
	!read(42, *) ((fc1_W_gga(i,j),j=1,126),i=1,1000)	
	!close(42)
	!open (42, file = 'fc2_W_gga.txt', status = 'old')
	!read(42, *) ((fc2_W_gga(i,j),j=1,1001),i=1,500)
	!close(42)
	!open (42, file = 'fcout_W_gga.txt', status = 'old')
	!read(42, *) ((fcout_W_gga(i,j),j=1,501),i=1,1)
	!close(42)
	open (42, file = 'enc1_W.txt', status = 'old') 
	read(42, *) ((enc1_W(i,j),j=1,3),i=1,100)	
	close(42)
	open (42, file = 'enc2_W.txt', status = 'old')
	read(42, *) ((enc2_W(i,j),j=1,101),i=1,100)
	close(42)
	open (42, file = 'enc3_W.txt', status = 'old')
	read(42, *) ((enc3_W(i,j),j=1,101),i=1,100)
	close(42)
	open (42, file = 'enc4_W.txt', status = 'old')
	read(42, *) ((enc4_W(i,j),j=1,101),i=1,100)
	close(42)
	open (42, file = 'encout_W.txt', status = 'old')
	read(42, *) ((encout_W(i,j),j=1,101),i=1,1)
	close(42)
	open (42, file = 'dec1_W.txt', status = 'old') 
	read(42, *) ((dec1_W(i,j),j=1,6),i=1,100)	
	close(42)
	open (42, file = 'dec2_W.txt', status = 'old')
	read(42, *) ((dec2_W(i,j),j=1,101),i=1,100)
	close(42)
	open (42, file = 'dec3_W.txt', status = 'old')
	read(42, *) ((dec3_W(i,j),j=1,101),i=1,100)
	close(42)
	open (42, file = 'dec4_W.txt', status = 'old')
	read(42, *) ((dec4_W(i,j),j=1,101),i=1,100)
	close(42)
	open (42, file = 'decout_W.txt', status = 'old')
	read(42, *) ((decout_W(i,j),j=1,101),i=1,1)
	close(42)
  end if
    ...
    if(der%mesh%use_curvilinear) then
      ...
      do ip = ipstart, ipend
		if (normalization) then
			!enc_inp_matrix_gga(1,1) = (dens(ip, 1) - density_mean)/density_std
			!enc_inp_matrix_gga(2,1) = (input_sigma(ip, 1) - sigma_mean)/sigma_std
			enc_inp_matrix_gga(1,1) = log10(dens(ip,1))
			enc_inp_matrix_gga(2,1) = log10(input_sigma(ip, 1))
		else
			enc_inp_matrix_gga(1,1) = dens(ip,1)
			enc_inp_matrix_gga(2,1) = input_sigma(ip, 1)
		end if
		call gga_encoder(enc_inp_matrix_gga, enc1_W, enc2_W, enc3_W, enc4_W, encout_W, enc_out_matrix_gga)
		!exc clip
		!if (enc_out_matrix_gga(1,1) > 0.05) then
		!	enc_out_matrix_gga(1,1) = 0.05
		!end if
		!if (enc_out_matrix_gga(1,1) < -2.0) then
		!	enc_out_matrix_gga(1,1) = -2.0
		!end if
		ec_per_vol(ip) = dens(ip,1)*enc_out_matrix_gga(1,1)
		ec_per_part(ip) = enc_out_matrix_gga(1,1)
  ...
  if (gga) then
	  do ip = 1, der%mesh%np
		dec_inp_matrix_gga(1,1) = ec_per_part(ip)
		if (normalization) then
			dec_inp_matrix_gga(2,1) = (dens(ip,1) - density_mean)/density_std
			dec_inp_matrix_gga(3,1) = (input_sigma(ip,1) - sigma_mean)/sigma_std
			dec_inp_matrix_gga(4,1) = (input_scalars(ip,1) - scalars_mean)/scalars_std
			dec_inp_matrix_gga(5,1) = (input_laplacian(ip,1) - laplacians_mean)/laplacians_std
		else
			dec_inp_matrix_gga(2,1) = dens(ip,1)
			dec_inp_matrix_gga(3,1) = input_sigma(ip,1)
			dec_inp_matrix_gga(4,1) = input_scalars(ip,1)
			dec_inp_matrix_gga(5,1) = input_laplacian(ip,1)
		end if
...
contains
  ...
  subroutine preprocess_input(array, m, out_array)
	integer, intent(in) :: m
	FLOAT, intent(in) :: array(:,:)
	FLOAT, intent(out) :: out_array(:,:)
	integer :: c, d, arrsize
	PUSH_SUB(xc_get_vxc.preprocess_input)
	arrsize = m + 1
	
	do c = 2, arrsize
		do d = 1, 1
			out_array(c, d) = array(c-1, d)
		end do
	end do
	out_array(1,1) = 1
	POP_SUB(xc_get_vxc.preprocess_input)
  end subroutine preprocess_input
  
  subroutine relu(array, m, n, out_array)
	integer, intent(in) :: m
	integer, intent(in) :: n
	FLOAT, intent(in) :: array(:,:)
	FLOAT, intent(out) :: out_array(:,:)
	integer:: c, d
	PUSH_SUB(xc_get_vxc.relu)
	do c = 1, m
		do d = 1, n
			if (array(c,d) > 0) then
				out_array(c,d) = array(c,d)
			else
				out_array(c,d) = 0
			end if
		end do
	end do
	POP_SUB(xc_get_vxc.relu)
  end subroutine relu
  
  subroutine elu(array, m, n, out_array)
	integer, intent(in) :: m
	integer, intent(in) :: n
	FLOAT, intent(in) :: array(:,:)
	FLOAT, intent(out) :: out_array(:,:)
	integer:: c, d
	PUSH_SUB(xc_get_vxc.elu)
	do c = 1, m
		do d = 1, n
			if (array(c,d) > 0) then
				out_array(c,d) = array(c,d)
			else
				out_array(c,d) = exp(array(c,d)) - 1
			end if
		end do
	end do
  ...
  subroutine gga_encoder(inp, enc1_W, enc2_W, enc3_W, enc4_W, encout_W, out_array)
	FLOAT, intent(in) :: inp(:,:)
	FLOAT, intent(in) :: enc1_W(:,:)
	FLOAT, intent(in) :: enc2_W(:,:)
	FLOAT, intent(in) :: enc3_W(:,:)
	FLOAT, intent(in) :: enc4_W(:,:)
	FLOAT, intent(in) :: encout_W(:,:)
	FLOAT, intent(out) :: out_array(:,:)
	FLOAT :: input_preprocessed(3,1)
	FLOAT :: part1_preprocessed(101, 1)
	FLOAT :: part2_preprocessed(101, 1)
	FLOAT :: part3_preprocessed(101, 1)
	FLOAT :: part4_preprocessed(101, 1)
	FLOAT :: part1(100, 1)
	FLOAT :: part1_act(100, 1)
	FLOAT :: part2(100, 1)
	FLOAT :: part2_act(100, 1)
	FLOAT :: part3(100, 1)
	FLOAT :: part3_act(100, 1)
	FLOAT :: part4(100, 1)
	FLOAT :: part4_act(100, 1)
	PUSH_SUB(xc_get_vxc.gga_encoder)
	call preprocess_input(inp, 2, input_preprocessed)
	part1 = matmul(enc1_W, input_preprocessed)
	call elu(part1, 100, 1, part1_act)
	call preprocess_input(part1_act, 100, part1_preprocessed)
	part2 = matmul(enc2_W, part1_preprocessed)
	call elu(part2, 100, 1, part2_act)
	call preprocess_input(part2_act, 100, part2_preprocessed)
	part3 = matmul(enc3_W, part2_preprocessed)
	call elu(part3, 100, 1, part3_act)
	call preprocess_input(part3_act, 100, part3_preprocessed)
	part4 = matmul(enc4_W, part3_preprocessed)
	call elu(part4, 100, 1, part4_act)
	call preprocess_input(part4_act, 100, part4_preprocessed)
	out_array = matmul(encout_W, part4_preprocessed)
	POP_SUB(xc_get_vxc.gga_encoder)
  end subroutine gga_encoder

  subroutine gga_decoder(inp, dec1_W, dec2_W, dec3_W, dec4_W, decout_W, out_array)
	FLOAT, intent(in) :: inp(:,:)
	FLOAT, intent(in) :: dec1_W(:,:)
	FLOAT, intent(in) :: dec2_W(:,:)
	FLOAT, intent(in) :: dec3_W(:,:)
	FLOAT, intent(in) :: dec4_W(:,:)
	FLOAT, intent(in) :: decout_W(:,:)
	FLOAT, intent(out) :: out_array(:,:)
	FLOAT :: input_preprocessed(6,1)
	FLOAT :: part1_preprocessed(101, 1)
	FLOAT :: part2_preprocessed(101, 1)
	FLOAT :: part3_preprocessed(101, 1)
	FLOAT :: part4_preprocessed(101, 1)
	...
	FLOAT :: part4(100, 1)
	FLOAT :: part4_act(100, 1)
	PUSH_SUB(xc_get_vxc.gga_decoder)
	call preprocess_input(inp, 5, input_preprocessed)
	part1 = matmul(dec1_W, input_preprocessed)
	call elu(part1, 100, 1, part1_act)
	call preprocess_input(part1_act, 100, part1_preprocessed)
	part2 = matmul(dec2_W, part1_preprocessed)
	call elu(part2, 100, 1, part2_act)
	call preprocess_input(part2_act, 100, part2_preprocessed)
	part3 = matmul(dec3_W, part2_preprocessed)
	call elu(part3, 100, 1, part3_act)
	call preprocess_input(part3_act, 100, part3_preprocessed)
	part4 = matmul(dec4_W, part3_preprocessed)
	call elu(part4, 100, 1, part4_act)
	call preprocess_input(part4_act, 100, part4_preprocessed)
	out_array = matmul(decout_W, part4_preprocessed)
	POP_SUB(xc_get_vxc.gga_decoder)
  end subroutine gga_decoder
  ...
  subroutine lda_init()
    ...
    if(calc_energy) then
	  ...
	  SAFE_ALLOCATE(ec_per_part(1:der%mesh%np))
      ex_per_vol = M_ZERO
      ec_per_vol = M_ZERO
	  ec_per_part = M_ZERO
    end if
