...
subroutine X(mesh_to_cube)(mesh, mf, cube, cf, local)
  ...
  integer :: z_pointer, ip_z_pointer
  logical :: local_
  ...
  if(.not. cf%in_device_memory) then
	...
	if(.not. order_file_exists) then
		open (43, file = 'order.txt', status = 'new')
	end if
	
	print*, "Map function starts..."
    ...
    do im = 1, mesh%cube_map%nmap
	  ...
	  !print*, ix, iy, iz, ip
	  if(.not. order_file_exists) then
		write(43, *) ix, iy, iz, ip
	  end if
	  ip_z_pointer = ip + 1
	  do z_pointer = 2, nn
		!print*, ix, iy, z_pointer, ip_z_pointer
		if(.not. order_file_exists) then
			write(43, *) ix, iy, z_pointer, ip_z_pointer
		end if
		ip_z_pointer = ip_z_pointer + 1
	  end do
      forall(ii = 0:nn - 1) cf%X(rs)(ix, iy, iz + ii) = gmf(ip + ii)
    end do
    !$omp end parallel do
	print*, "Map function ends..."
	close(43)
